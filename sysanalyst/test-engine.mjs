/* End-to-end engine test with a minimal DOM/localStorage shim.
   Run with:  node sysanalyst/test-engine.mjs

   Loads the REAL config.js + questions.js + deep.js + app.js and drives
   full attempts through a fake DOM. Verifies: free-test grading, the
...
// key by prompt + options (some prompts intentionally repeat across forms;
// the options make the pair unique).
...
function driveExam(mode) {
  ...
  while ($("screen-exam").classList.contains("active") && guard++ < 200) {
    ...
    const shownOptions = qEl.querySelectorAll(".option").map((b) => b.children[1].innerHTML);
...
// the paywall section left us at 1 completed test (form-1); add free tests to reach 3
