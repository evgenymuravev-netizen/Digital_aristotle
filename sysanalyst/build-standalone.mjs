/* Bundle the SAA app into a single self-contained HTML file.
   Run with:  node sysanalyst/build-standalone.mjs
   Inlines styles.css + config.js + questions.js + deep.js + app.js into
   index.html so the whole test works by double-clicking (no server, no CORS).

   Outputs:
     sysanalyst/standalone.html          — full document (repo / file://), links intact
     <scratchpad>/saa-artifact.html      — body-only build for the Artifact renderer
                                           (cross-page links stripped) */
...
console.log("saa-artifact.html: " + (artifact.length / 1024).toFixed(0) + " KB, stray local refs: " + strayArt);
