...
const replicaBar = () => CN.replica
  ? `<div style="background:#0e0e10;color:#fff;border-radius:12px;padding:9px 13px;font:600 11px Inter,sans-serif;display:flex;gap:8px;align-items:center;margin-bottom:14px">
      🎬 1:1 flow replica of the Lean Link recording — <span style="color:var(--lime)">✦ gold = Noor enhancement</span></div>` : '';
const enh = t => `<span class="tag" style="background:rgba(127,176,255,.25);color:#45454d;margin-left:6px">✦ ${t}</span>`;
...
SCREENS['connect-intro'] = () => {
  ...
  return `
  <div class="scr">
    <div class="glowblob" style="background:#7FB0FF;top:-60px;right:-100px;opacity:.22"></div>
...
SCREENS['connect-banks'] = () => {
  ...
  <div class="scr light">
    ...
    <div class="card mt16 tap" onclick="A.go('connect-statements')" style="border-color:rgba(255,103,87,.35)">
...
SCREENS['connect-statements'] = () => {
  ...
  <div class="scr light">
    ...
    <div class="card soft mt12"><div class="micro">This account now behaves like a linked one — balances and categories in your money picture, counted into Zakat and net worth. Refresh monthly by re-upload, or forward statements to <b>khadeeja.statements@noor.finance</b> and the inbox watchdog does it for you.</div></div>
...
SCREENS['connect-fallback'] = () => `
  <div class="scr light">
    ${hdr('',{right:'<button class="chip" onclick="A.go(\'connect-intro\')">Close</button>'})}
    ${replicaBar()}
    <div class="card" style="background:rgba(127,176,255,.2);border:1px solid #dfa92e;border-radius:18px">
      <div class="flex" style="gap:10px">${aiIc(15)}<b style="font-size:14px">Noor enhancement — no dead-ends</b></div>
      <div class="micro mt8" style="color:#45454d">Camera failed, so Noor Connect <b>automatically fell back to SMS verification</b>. The user never sees an error wall — the recording ended on “Wio couldn’t verify your identity”; we recover instead.</div>
    </div>
...
SCREENS['connect-finale'] = () => `
  <div class="scr center">
    <div class="glowblob" style="background:#7FB0FF;top:-70px;left:-80px;opacity:.2"></div>
