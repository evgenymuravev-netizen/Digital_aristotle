...
window.BANKS = {
  ...
  noor:    {name:'Noor',             full:'Noor Wallet',               bg:'#0e0e10', fg:'#0e0e10', accent:'#0e0e10', short:'m'},
  /* digital wallets */
...
window.blg = (id, size='') => {
  if (window.BRAND_SVG && (BRAND_SVG[id] || (window.BRAND_WORD && BRAND_WORD[id]))) {
    ...
    return `<span class="blg ${size}" style="background:#fff;border:1px solid rgba(14,14,16,.08);color:${w.h};font:700 ${Math.max(6,Math.round(w.fs*px/44))}px Outfit,Inter,sans-serif;letter-spacing:-.02em">${w.t}</span>`;
  }
  if (window.BRAND_SVG && (BRAND_SVG[id] || (window.BRAND_WORD && BRAND_WORD[id]))) {
    const px = size==='xs'?24:size==='sm'?30:44;
    if (BRAND_SVG[id]) return `<span class="blg ${size}" style="background:#fff;border:1px solid rgba(14,14,16,.08)"><svg viewBox="0 0 24 24" width="${Math.round(px*0.55)}" height="${Math.round(px*0.55)}"><path fill="${BRAND_SVG[id].h}" d="${BRAND_SVG[id].p}"/></svg></span>`;
...
window.CATS = {
  ...
  transport:{n:'Transport', ic:'car',  c:'#FF6757'},
  shopping: {n:'Shopping',  ic:'gift', c:'#E2543F'},
...
/* ---------- avatar palette ---------- */
window.AVC = ['#FF6757','#E2543F','#c85a8e','#1f8a5b','#c8841f','#2f8f83','#d1483f'];
...
window.ILL = {
  shieldBig: `<div style="width:92px;height:92px;border-radius:30px;background:linear-gradient(135deg,#A9C9FF,#0e0e10 60%,#d69f22);display:flex;align-items:center;justify-content:center;color:#0e0e10;box-shadow:0 18px 50px rgba(127,176,255,.35)">${ic('shieldCheck',46,'',1.6)}</div>`,
