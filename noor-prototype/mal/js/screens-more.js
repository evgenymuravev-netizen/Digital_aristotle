...
SCREENS.vcard = () => `
  <div class="scr">
    ...
    <div class="ccvisual" style="background:linear-gradient(130deg,#4a3a10,#1c1504 70%)">
      <div class="flex between"><span style="font:600 17px Onest,Inter,sans-serif;letter-spacing:-.02em;color:#fff">Mal</span><span class="cc-small">Virtual · disposable</span></div>
      <div class="cc-chip"></div>
...
SCREENS.paybill = (id) => {
  ...
  <div class="scr">
    ...
    <div class="card mt12" style="border-color:rgba(166,168,253,.4)">
    ...
    <button class="btn lime mt16" onclick="confetti(document.getElementById('screen'));A.toast('Paid — +120 Mal pts earned 🎉','gift');setTimeout(()=>A.go('rewards'),900)">
...
SCREENS.dispute = () => `
  <div class="scr">
    ...
    <div class="card soft mt12"><div class="micro">Mal files the dispute with FAB on your behalf and tracks it. Typical waiver rate for first-time fees: 84%.</div></div>
...
SCREENS.statement = () => `
  <div class="scr">
    ...
    <div class="listcard">
      ${[['May 2026 statement','PDF · all accounts combined'],['April 2026 statement','PDF'],['Consent receipts (4)','Mal Connect artefacts'],['Zakat certificate 1447H','For your records'],['Salary certificates','Auto-generated from verified income']]
...
SCREENS.apply = (id) => {
  ...
  <div class="scr">
    ...
    <div class="psteps mt8" id="apSteps">
      ${['Income verified via Mal Connect','AECB soft-check — no score impact','KYC reused from your Mal profile',(o.shariah?'Shariah contract (Tawarruq) prepared':'Card agreement prepared')]
  ...
  <div class="scr">
    ...
    <div class="card mt12">
      ...
      <div class="flex mt12" style="gap:7px">
        ${['M','T','W','T','F','S','S'].map((d,i)=>`<div style="flex:1;text-align:center"><div class="bigico" style="width:100%;height:40px;border-radius:12px;background:${i<5?'rgba(166,168,253,.2)':'var(--glass)'};color:${i<5?'var(--lime)':'var(--tx3)'}">${i<5?ic('check',16):d}</div></div>`).join('')}
      </div>
    </div>
...
AFTER.scratch = () => {
  ...
  x.fillStyle='rgba(166,168,253,.8)'; x.font='700 15px Inter,sans-serif'; x.textAlign='center';
...
SCREENS.zakat = () => {
  ...
  if(step===2){
    ...
    <div class="scr">
      ...
      ${z.charityInfo.map((c,i)=>`
        <div class="card mt12 tap" style="${sel===i?'outline:2px solid var(--lime)':''}" onclick="ZK.set('charity',${i})">
          <div class="flex" style="gap:12px">
            <span class="avx" style="background:rgba(166,168,253,.14);font-size:24px">${c.em}</span>
      ...
      <div class="card soft mt12 flex" style="gap:10px">${ic('shieldCheck',20,'lime-t')}
        <div class="micro"><b>Mal takes 0%.</b> Every receipt lands in your Documents within minutes, with the charity’s zakat-fund reference — auditable end-to-end.</div></div>
  ...
  if(step===3){
    ...
    const PLANS = [
      ...
      ['later','Pay later — Mal advances it (Qard Ḥasan)', `Charity receives 100% today · you repay 4 × AED ${fm(due/4)} · 0 fees — zakat is never late`],
      ['income','A % of each salary', `${pct}% = AED ${fm(perSal,0)}/salary · finishes by ${finish} — Mal tops up any gap at hawl-end`],
    ...
    <div class="scr">
      ...
      <div class="lbl mt16 mb8">Choose a plan ${tipi('All five plans end with zakat fully paid on time. Qard Hasan = Mal pays the charity today and you repay an interest-free advance; the Pot saves ahead for NEXT year.')}</div>
      <div class="listcard">
        ${PLANS.map(([id,tt,dd])=>`
      ...
      ${plan==='later'?`<div class="micro mt8">Fiqh note: delaying zakat without need is discouraged — so Mal pays it <b>on time, in full</b> as an interest-free loan to you (Qard Ḥasan). Your repayment is a debt, not zakat.</div>`:''}
  ...
  return `
  <div class="scr">
    ${hdr('Zakat · 1447H',{right:`<button class="ai-orb" style="width:40px;height:40px;min-width:40px;border:none;cursor:pointer" onclick="chatDeep('zakatFull')" title="Ask Mal">${ic('spark',19)}</button>`})}
    ${stepper}
    <div class="card lime">
      <div class="flex between"><b style="font-size:14px">🌙 ${z.hijri}</b></div>
      <div class="micro mt4">Anchor your hawl to 1 Ramadan and pay tomorrow — Mal recalculates on the day at live prices.</div>
    </div>
    ...
    <button class="chip mt8" onclick="chatDeep('zakatFull')">✦ Not sure? Mal interviews you & matches your scholar</button>
    ...
    <div class="lbl mt16 mb8">Seen by Mal (live)</div>
    <div class="listcard">
...
SCREENS.approvals = () => {
  ...
  const row = (it) => {
    ...
    <div class="card mt8" style="${dec==='no'?'opacity:.5':''}">
      <div class="flex" style="gap:12px">
        <span class="avx" style="background:rgba(166,168,253,.12);font-size:22px">${it.em}</span>
...
SCREENS.agents = () => {
  ...
  <div class="scr">
    ...
    ${AGENTS.fleet.map(f=>`
      <div class="card mt8">
        <div class="flex" style="gap:12px">
          <span class="avx" style="background:rgba(166,168,253,.12);font-size:22px">${f.em}</span>
...
const BIZ_CARDS = [
  ...
  {ic:'💳', icBg:'#D8F8EA',
   ...
   chart:`<div style="display:flex;align-items:flex-end;justify-content:center;gap:16px;height:180px;margin:16px 0 4px">
    ...
    <div class="chips" style="justify-content:center">
      ...
      <span class="tag" style="background:#EFE2FF;color:#6B2BD9">Mal card</span>
  ...
  {ic:'👕', icBg:'#EFE2FF',
   ...
   src:'Based on Mal merchant data', cta:'Show this in a week'},
];
SCREENS.biz = () => {
...
SCREENS.debts = () => {
  ...
  <div class="scr">
    ...
    <div class="card">
      ...
      <div class="micro mt4">9 facilities across 4 providers · auto-detected via Mal Connect</div>
      <div class="hr"></div>
  ...
  <div class="scr">
    ...
    ${DEBTS.map(d=>{
      ...
      <div class="card mt12">
        ...
        ${lens&&d.malRev?`<div class="micro" style="color:var(--gold)">💰 Mal earns ≈ AED ${fm(d.malRev,0)} (1% switch fee + margin share)</div>`:lens?`<div class="micro" style="color:var(--tx3)">💰 Mal earns AED 0 — trust play</div>`:''}
...
SCREENS.dsf = () => `
  <div class="scr">
    ...
    <div class="card" style="text-align:center;background:linear-gradient(150deg,rgba(166,168,253,.12),var(--glass))">
      <div style="font-size:40px">🔐</div>
    ...
    <div class="card soft mt12 flex" style="gap:10px">${ic('moon',18,'gold-t')}<div class="micro">Structure: commodity Murabaha with a pledge (rahn) over the deposit — reviewed by the Mal Shariah board; no interest at any step.</div></div>
...
SCREENS.consents = () => `
  <div class="scr">
    ...
    <div class="card soft mt12 flex" style="gap:10px">${ic('shieldCheck',20,'lime-t')}<div class="micro">Mal never sells data. Access logs are auditable below — 142 reads this month, all by you or your rules.</div></div>
  ...
  <div class="scr">
    ...
    <div class="card flex" style="gap:14px">
      ...
      <div class="f1"><div class="h3">${USER.first} ${USER.last}</div>
        <div class="micro mt4">${USER.phone} · with Mal since ${USER.since}</div>
...
SCREENS['agentic-match'] = () => `
  <div class="scr">
    ...
    <div class="card lime">
      ...
      <div class="sub mt8" style="color:rgba(14,14,16,.66)">FAB refreshed their Cashback Visa campaign yesterday at 14:05. Your Mal Agentic card matched it by 15:02 — applied only to you.</div>
    </div>
    <div class="lbl mt20 mb8">What FAB launched — and what you now have on Mal</div>
    <div class="listcard">
      ${[['5% cashback on groceries & fuel','was 1.5% on Mal','check'],
         ['Double cashback on dining this month','new — matched same-day','check'],
         ['Cinema buy-1-get-1 weekends','matched via Mal Perks','check'],
         ['Annual fee AED 0','always true on Mal','check']]
    ...
    <div class="card soft mt12"><div class="micro">How this works: agents watch every card campaign in the UAE — including banks you already hold. When one of your cards gets better, Mal matches the benefit on your Agentic card within the hour, or simply tells you to pay with theirs. Whichever wins for you. That is the promise: agentically better.</div></div>
    <div class="btnrow mt16">
      <button class="btn lime" onclick="A.toast('Matched benefits active until 31 Jul','check')">Use Mal for these</button>
