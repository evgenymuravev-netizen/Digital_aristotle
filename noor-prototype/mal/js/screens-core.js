...
SCREENS['accounts-iban'] = () => `
  <div class="scr">
    ${hdr('My IBAN & details')}
    <div class="card">
      <div class="lbl">Mal wallet · AED</div>
      <div class="h3 mt8 tnum">${USER.iban}</div>
      <div class="micro mt4">${USER.first} ${USER.last} · Mal Financial Platform</div>
      <div class="btnrow mt12">
    ...
    <div class="micro mt12">Salary? Share this IBAN with HR — Mal routes it and runs your salary rules automatically.</div>
  </div>`;
...
SCREENS.money = () => {
  ...
  <div class="scr" style="padding-bottom:170px">
    ...
    <div class="micro mt12" style="text-align:center">Banks, wallets, BNPL and crypto — aggregated read-only via Mal Connect (CBUAE Open Finance).</div>
  </div>`;
};
window.Money = {
  sync(){
...
SCREENS.account = (id) => {
  ...
  <div class="scr">
    ...
    ${isCard?`<div class="card lime mt12 tap" onclick="A.go('paybill/${a.id}')">
      <div class="flex between"><b style="font-size:14px">Pay card bill — earn 120 Mal pts</b>${ic('chevR',18)}</div>
...
SCREENS.txn = (id) => {
  ...
  <div class="scr">
    ...
    <div class="chips">${Object.keys(CATS).slice(0,8).map(k=>`<button class="chip ${k===t.cat?'on':''}" onclick="A.toast('Recategorised — Mal learns this merchant','check')">${CATS[k].n}</button>`).join('')}</div>
...
SCREENS.bank = (id) => {
  ...
  <div class="scr">
    ${hdr(B.name)}
    <div class="card flex" style="gap:14px">${blg(id)}<div class="f1"><div class="h3">${B.full}</div><div class="micro mt4">Connected via Mal Connect · ${cs?cs.freq:'4× / day'} refresh</div></div><span class="tag grn">Live</span></div>
