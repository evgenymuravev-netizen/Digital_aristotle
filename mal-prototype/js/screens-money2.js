...
window.Pay = {
  ...
  ps5Done(store, price){
    ...
    if (document.getElementById('chWrap')) {
      ...
      Chat.push({from:'ai', html:`Done ✅ Ordered from <b>${store}</b> ${how}. I compared Tabby, Tamara and Mal Split — all pre-approved; ${sel.id==='noor'?'Mal Split won on total cost':'your pick is locked in'}. Tracking lands in your briefing tomorrow.`});
    }
  },
};
...
SCREENS.request = () => `
  <div class="scr">
    ...
    <div class="card" style="text-align:center">
      ...
      <div style="width:170px;height:170px;margin:14px auto;border-radius:18px;background:#fff;display:flex;align-items:center;justify-content:center;color:#0e0e10">${ic('qr',120,'',1)}</div>
    ...
    <div class="btnrow mt12">
      <button class="btn ghost" onclick="A.toast('Payment link copied: mal.ai/p/john','link')">${ic('link',18)} Copy link</button>
...
SCREENS.qr = () => `
  <div class="scr nopad" style="background:#060819;display:flex;flex-direction:column;padding:64px 18px 40px">
...
SCREENS.intl = () => {
  ...
  <div class="scr">
    ...
    <div class="listcard mt12">
      ...
      <div class="kv" style="padding:11px 2px"><span class="k">Mal fee</span><span class="v grn-t">AED 0 — free</span></div>
...
SCREENS.insights = () => {
  ...
  <div class="scr" style="padding-bottom:170px">
    ...
    ${(()=>{ /* Apple-Watch-style budget rings: plan vs fact per ring */
      ...
      const R=[{n:'Essentials',d:ess,c:'#1f8a5b'},{n:'Lifestyle',d:life,c:'#c8841f'},{n:'Travel & other',d:trav,c:'#2f8f83'}];
    ...
    <div class="listcard">
      ${CATSPEND.map(c=>{ const p=c.amt/c.bud, tc=p>1?'#d1483f':p>=0.85?'#c8841f':'#1f8a5b'; return `
        <div class="row" onclick="A.go('cat/${c.cat}')">
          ...
          <div class="row-r"><div class="row-amt tnum" style="color:${p>1?'#d1483f':'var(--tx)'}">${fm(c.amt,0)}</div><div class="row-sub tnum">of ${fm(c.bud,0)}</div></div>
        </div>`;}).join('')}
    </div>
    ...
    const share = s.share ? `
      <div class="card mt8" style="border-color:rgba(31,138,91,.45)">
        ...
        <div class="btnrow mt8">
          ...
          <button class="btn pri sm" onclick="A.closeSheet();A.toast('Cancellation queued for ${s.share.who}’s iCloud — pending her OK in her Mal app','check')">Cancel hers — with her OK</button>
        </div>
      </div>` : '';
    const music = s.music ? `
      <div class="card soft mt12">
        ...
        ${meter(Math.min(s.hours/31,1), s.trend>0?'#1f8a5b':'#d1483f')}
...
SCREENS.roundups = () => `
  <div class="scr">
    ...
    <div class="listcard mt12">
      ...
      <div class="row static"><div class="row-main"><div class="row-t">Destination</div></div><span class="tag gold">Mal Gold (vaulted, 999.9)</span></div>
    </div>
...
SCREENS.rules = () => `
  <div class="scr">
    ${hdr('Mal Rules',{right:`<button class="chip" onclick="A.go('rule-new')">${ic('plus',14)} New</button>`})}
...
window.Insights = {
  budgetSheet(id){
    A.sheet(`<div class="h2">Budget for ${CATS[id].n}</div>
      ...
      <div class="micro mt12">Mal warns you at 80% and can auto-freeze the category at 100%.</div>`);
  }
};
})();
