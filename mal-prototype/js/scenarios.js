...
{g:'Debt intelligence ✦', items:[
  ...
  S('SME — documents & UBOs','Licence, MoA, AoA as separate captures; every UBO adds EID + passport',()=>{A.tmp.smev=4;A.tmp.smeDocs={};A.go('sme-video');}),
  S('SME — Ejari & web presence','Ejari mandatory (personal works for solo); site, socials & Tabby/Tamara back-office raise approval',()=>{A.tmp.smev=6;A.tmp.smeDocs={};A.go('sme-video');}),
  S('SME — the offer, AED 100,000','Approved terms: 18% fixed profit (AED 18,000), 2,950/week × 40 weeks, Murabaha on Platinum',()=>{A.tmp.smev=7;A.go('sme-video');}),
  S('SME — Murabaha signing cascade','Ten documents, eleven seconds: offer → certificates → your contract → notice of sale, all timestamped',()=>{A.tmp.smev=8;A.tmp.smeSigned=0;A.go('sme-video');}),
  S('SME — direct debits & release','Wio business mandate + ENBD personal backstop → AED 100,000 released',()=>{A.tmp.smev=9;A.tmp.smeDD={};A.go('sme-video');}),
