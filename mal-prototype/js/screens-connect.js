/* ============ Mal Connect — Lean-style linking flow (copied from the Apr 2026 recording, enhanced) ============ */
(function(){
...
SCREENS['connect-intro'] = () => {
  ...
  <div class="scr">
    ...
    <div class="micro mt8">Investment accounts? Mal suggests them right after onboarding — no need to do everything now.</div>
    <button class="btn lime mt20" onclick="${linked.length?'CN.finale()':'CN.start()'}">${linked.length?'Continue':'Connect first bank'}</button>
    <button class="btn ghost mt8" onclick="A.sheet(CN.dataSheet())">Learn what data Mal uses</button>
  </div>`;
};
CN.toggleBank = b => { A.S.linked.includes(b) ? null : CN.start(b); };
CN.dataSheet = () => `
  <div class="h2">What Mal reads — and what it can’t</div>
  <div class="mt12">
    ${[['check','Account names, balances','Refreshed up to 4× a day'],
       ...
       ['x','Your bank password','Exchanged directly with your bank — never visible to Mal'],
  ...
  <div class="scr light">
    ...
    <div class="mt20">
      ...
      <div class="row static">${ic('shieldCheck',20)}<div class="row-main"><div class="row-t" style="font-size:13.5px">Regulated in the UAE.</div>
        <div class="row-d" style="white-space:normal">Mal Connect is licensed by the Central Bank of the UAE under the Open Finance Regulation, licence OF-200133.</div></div></div>
...
SCREENS['connect-login'] = (bank) => {
  ...
  <div class="scr light">
    ...
    <div class="micro mt16" style="text-align:center;line-height:1.6">Mal Connect is regulated by the Central Bank of the UAE. Your credentials are exchanged <b>directly with ${B.name}</b> and are never visible to or stored by Mal ${enh('vs. credential sharing')}. By continuing, you agree to the Privacy Notice and End User Agreement.</div>
    ${pby()}
  </div>`;
};
CN.typeUser = () => {
...
SCREENS['connect-consent'] = (bank) => `
  <div class="scr light">
    ...
    <div class="card white mt20" style="border:1px solid var(--lt-line)">
      <div class="kv"><span class="k">You allow</span><span class="v">Mal · Mal Connect</span></div>
