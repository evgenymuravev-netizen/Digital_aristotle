...
{g:'First run & onboarding', items:[
  S('Splash & brand reveal','Noor wordmark on midnight indigo, auto-advances','splash','fresh'),
...
{g:'Products, wealth & financing', items:[
  ...
  S('Agentically better — promo matched','FAB improved their card yesterday; Noor matched it in an hour, only for you',()=>{A.go('home');setTimeout(()=>iosNotif('Agentically better','FAB upgraded their card campaign — your Noor Agentic card already matches it. Tap to see.',`<span class="bigico" style="width:38px;height:38px;min-width:38px;border-radius:9px;background:#FFEDEB;color:#FF6757">${ic('zap',20)}</span>`,()=>A.go('agentic-match')),700);}),
...
{g:'Debt intelligence ✦', items:[
  ...
  S('Qard Hasan — draw the money','Salary already at Noor: pick the exact amount, confirm, repaid on the 25th',()=>{A.tmp.qardActive=1;A.tmp.qardAmt=2140;A.go('qard');}),
  ...
  S('SME financing by video','Docs first, the 15-min plan video last — uploads for papers, camera-only for the story',()=>{A.tmp.smev=1;A.tmp.smevRec=0;A.go('sme-video');}),
  S('SME — who signs + reused KYC','After the AoA: pick the signatory; Khadeeja already passed retail KYC — nothing to upload',()=>{A.tmp.smev=3;A.tmp.smeSig=0;A.go('sme-video');}),
  S('SME — documents & UBOs','Licence, MoA, AoA as PDF uploads first; non-signatory UBOs upload EID + passport too',()=>{A.tmp.smev=2;A.tmp.smeSig=0;A.tmp.smeDocs={};A.go('sme-video');}),
  S('SME — Ejari, read from the licence','LLC licence → business Ejari asked automatically; site, socials & BNPL back-office raise approval',()=>{A.tmp.smev=5;A.tmp.smeSolo=0;A.tmp.smeDocs={};A.go('sme-video');}),
  S('SME — sole-establishment fork','The licence says Sole Establishment → the journey asks for personal Ejari instead. Never a question to answer',()=>{A.tmp.smev=5;A.tmp.smeSolo=1;A.tmp.smeDocs={};A.go('sme-video');}),
  S('SME — the offer, AED 100,000','Approved terms: 18% fixed profit (AED 18,000), 2,950/week × 40 weeks, Murabaha on Platinum',()=>{A.tmp.smev=8;A.go('sme-video');}),
  S('SME — Murabaha signing cascade','Ten documents, eleven seconds: offer → certificates → your contract → notice of sale, all timestamped',()=>{A.tmp.smev=9;A.tmp.smeSigned=0;A.go('sme-video');}),
  S('SME — direct debits & release','Wio business mandate + ENBD personal backstop → AED 100,000 released',()=>{A.tmp.smev=10;A.tmp.smeDD={};A.go('sme-video');}),
