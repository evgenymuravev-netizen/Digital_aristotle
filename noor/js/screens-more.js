...
const cardVisual = (a, mini=false) => `
  <div class="ccvisual ${A.S.frozen?'frozen':''}" style="background:${a.art};${mini?'height:150px':''}" onclick="A.go('card/${a.id}')">
    <div class="flex between"><span style="font:600 15px Inter,sans-serif">${BANKS[a.bank].name}</span><span class="cc-small">${a.name}</span></div>
    <div class="cc-chip"></div>
    <div>
      <div class="cc-num">•••• •••• •••• ${a.mask}</div>
      <div class="flex between mt8"><span class="cc-small">${USER.first} ${USER.last}</span><span style="font:600 14px Inter,sans-serif;font-style:italic">VISA</span></div>
    </div>
  </div>`;
SCREENS.cards = () => {
...
SCREENS.paybill = (id) => {
  ...
  <div class="scr">
    ...
    <div class="card mt12" style="border-color:rgba(127,176,255,.4)">
  ...
  <div class="scr center">
    ...
    <div class="mt16" style="width:100%">${`
      <div class="ccvisual" style="background:linear-gradient(130deg,#0A2E5C,#06182F 70%)">
        <div class="flex between"><span style="font:600 15px Inter,sans-serif">${BANKS[o.bank].name}</span><span class="cc-small">just issued</span></div>
        <div class="cc-chip"></div>
        <div><div class="cc-num">5402 33•• •••• 9018</div>
        <div class="flex between mt8"><span class="cc-small">${USER.first} ${USER.last}</span><span style="font:600 14px Inter,sans-serif;font-style:italic">VISA</span></div></div>
      </div>`}</div>
...
SCREENS.invest = () => `
  <div class="scr">
    ...
    <div class="card">
      ...
      <div class="flex between mt8"><span style="font:600 30px Inter,sans-serif" class="tnum">AED ${fm(INVEST.total,0)}</span><span class="tag grn">▲ ${INVEST.day}% today</span></div>
      <div class="mt12">${spark([31,32.4,31.8,33.5,34.1,35.2,36.4,37.1,38.5],330,70,'#1f8a5b')}</div>
    </div>
    <div class="listcard mt12">
...
SCREENS.gold = () => `
  <div class="scr">
    ...
    <div class="card" style="text-align:center;background:linear-gradient(150deg,rgba(200,132,31,.16),var(--glass))">
      ...
      <div style="font:600 32px Inter,sans-serif" class="tnum mt8">${INVEST.gold.grams} g</div>
      <div class="micro mt4">≈ AED ${fm(INVEST.gold.val)} · 999.9 vaulted in DMCC · Shariah-certified</div>
      <div class="mt12">${spark([440,452,448,461,458,470,479,486],300,60,'#c8841f')}</div>
...
SCREENS.score = () => `
  <div class="scr">
    ...
    <div class="card mt12">
      ...
      <div class="mt12">${spark(P.arr,330,64, P.end>=SCORE.v?'#1f8a5b':'#d1483f')}</div>
...
SCREENS.rewards = () => `
  <div class="scr">
    ${hdr('Rewards')}
    <div class="card lime">
      <div class="flex between"><span class="lbl" style="color:rgba(14,14,16,.55)">Noor points</span><span class="tag" style="background:rgba(14,14,16,.14);color:#0e0e10">${REWARDS.tier} tier</span></div>
      <div style="font:600 36px Inter,sans-serif" class="tnum mt8">${fm(REWARDS.pts,0)}</div>
    ...
    <div class="card mt12">
      ...
      <div class="flex mt12" style="gap:7px">
        ${['M','T','W','T','F','S','S'].map((d,i)=>`<div style="flex:1;text-align:center"><div class="bigico" style="width:100%;height:40px;border-radius:12px;background:${i<5?'rgba(127,176,255,.2)':'var(--glass)'};color:${i<5?'var(--lime)':'var(--tx3)'}">${i<5?ic('check',16):d}</div></div>`).join('')}
      </div>
    </div>
    <div class="lbl mt16 mb8">Scratch cards · ${REWARDS.scratch} waiting</div>
    <div class="grid2">
      <div class="tile" style="background:linear-gradient(135deg,#3a2d10,#1d1606);text-align:center" onclick="A.go('scratch')">
...
AFTER.scratch = () => {
  ...
  const grd=x.createLinearGradient(0,0,r.width,r.height); grd.addColorStop(0,'#4a3a12'); grd.addColorStop(1,'#231a06');
  x.fillStyle=grd; x.fillRect(0,0,r.width,r.height);
  x.fillStyle='rgba(127,176,255,.8)'; x.font='700 15px Inter,sans-serif'; x.textAlign='center';
...
SCREENS.zakat = () => {
      ...
      ${z.charityInfo.map((c,i)=>`
        <div class="card mt12 tap" style="${sel===i?'outline:2px solid var(--lime)':''}" onclick="ZK.set('charity',${i})">
          <div class="flex" style="gap:12px">
            <span class="avx" style="background:rgba(127,176,255,.14);font-size:24px">${c.em}</span>
  ...
  return `
  <div class="scr">
    ${hdr('Zakat · 1447H',{right:`<button class="ai-orb" style="width:40px;height:40px;min-width:40px;border:none;cursor:pointer" onclick="chatDeep('zakatFull')" title="Ask Noor">${ic('spark',19)}</button>`})}
    ${stepper}
    <div class="card lime">
    ...
    <div class="card mt16" style="background:linear-gradient(150deg,rgba(200,132,31,.16),var(--glass))">
      ...
      <div style="font:600 38px Inter,sans-serif" class="tnum mt8">AED ${fm(due)}</div>
...
SCREENS.approvals = () => {
  ...
  const row = (it) => {
    ...
    <div class="card mt8" style="${dec==='no'?'opacity:.5':''}">
      <div class="flex" style="gap:12px">
        <span class="avx" style="background:rgba(127,176,255,.12);font-size:22px">${it.em}</span>
  ...
  return `
  <div class="scr">
    ${hdr('Approvals',{right:`<button class="ai-orb" style="width:40px;height:40px;min-width:40px;border:none;cursor:pointer" onclick="chatDeep('agents')">${ic('spark',19)}</button>`})}
    <div class="card lime">
...
SCREENS.agents = () => {
  ...
  return `
  <div class="scr">
    ${hdr('Your agents',{right:`<button class="ai-orb" style="width:40px;height:40px;min-width:40px;border:none;cursor:pointer" onclick="chatDeep('agents')">${ic('spark',19)}</button>`})}
    <div class="card lime">
    ...
    <div class="card mt12 tap" onclick="A.go('approvals')" style="border-color:rgba(127,176,255,.4)">
    ...
    ${AGENTS.fleet.map(f=>`
      <div class="card mt8">
        <div class="flex" style="gap:12px">
          <span class="avx" style="background:rgba(127,176,255,.12);font-size:22px">${f.em}</span>
...
SCREENS.gift = () => {
  ...
  <div class="scr">
    ...
    <div class="card soft mt12 flex" style="gap:10px;align-items:flex-start">${ic('shieldCheck',20,'lime-t')}
      <div class="micro"><b>With his consent:</b> ${g.consent} ${tipi('These signals come from Omar’s own Noor app — he chose to share gift-relevant categories with you. You see ideas, never her raw transactions or searches.')}</div></div>
...
const BIZ_CARDS = [
  {ic:'🚀', icBg:'#FCE7F3',
   ...
   chart:`<div style="display:flex;align-items:flex-end;justify-content:center;gap:34px;height:170px;margin:18px 0 6px">
      <div style="text-align:center"><div class="micro" style="color:#6a6a73;font-weight:700">32K</div>
        <div style="width:84px;height:104px;background:#E935D8;border-radius:10px;margin-top:6px"></div>
        <div class="micro mt8" style="color:#6a6a73">Last month</div></div>
      <div style="text-align:center"><div class="micro" style="color:#111;font-weight:800">41.6K</div>
        ...
        <div class="micro mt8" style="color:#6a6a73">This month</div></div>
    </div>`,
   ...
   chart:`<div style="display:flex;align-items:flex-end;justify-content:center;gap:16px;height:180px;margin:16px 0 4px">
      ${[['Sep',38,[0,34,28]],['Oct',32,[0,40,28]],['Nov',18,[26,36,20]],['Dec',12,[38,34,16]]].map(([mn,rej,seg])=>`
        <div style="text-align:center">
          <div class="micro" style="color:${rej===12?'#fff':'#6a6a73'};font-weight:800;${rej===12?'background:#0e0e10;border-radius:8px;padding:2px 7px':''}">${rej}%</div>
          ...
          <div class="micro mt4" style="color:#6a6a73">${mn}</div></div>`).join('')}
    </div>
  ...
  {ic:'👕', icBg:'#EFE2FF',
   ...
   chart:`<div style="margin:26px 0 10px">
      <div class="kv" style="font-size:15px"><span class="k" style="color:#6a6a73">Average CAC</span><span class="v tnum" style="font-size:22px;font-weight:800;color:#9a9aa2">AED 3.20</span></div>
      <div class="kv" style="font-size:15px"><span class="k" style="color:#6a6a73">Your CAC</span><span class="v tnum" style="font-size:22px;font-weight:800;color:#111">AED 4.21</span></div>
    </div>`,
...
SCREENS.biz = () => {
  ...
  <div class="scr light" style="display:flex;flex-direction:column">
    ${hdr('Business insights',{right:`<span class="tag lime" style="color:#45454d">${i+1}/${BIZ_CARDS.length}</span>`})}
...
SCREENS.debts = () => {
  ...
  <div class="scr">
    ...
    <div class="card">
      ...
      <div style="font:600 32px Inter,sans-serif" class="tnum mt8">AED ${fm(totOut,0)}</div>
...
SCREENS.refi = () => {
  ...
  const saves = DEBTS.reduce((s,d)=>s+d.save,0), rev = DEBTS.reduce((s,d)=>s+d.malRev,0);
  ...
  <div class="scr">
    ...
    ${DEBTS.map(d=>{
      ...
      <div class="card mt12">
        ...
        ${lens&&d.malRev?`<div class="micro" style="color:var(--gold)">💰 Noor earns ≈ AED ${fm(d.malRev,0)} (1% switch fee + margin share)</div>`:lens?`<div class="micro" style="color:var(--tx3)">💰 Noor earns AED 0 — trust play</div>`:''}
...
SCREENS.dsf = () => `
  <div class="scr">
    ...
    <div class="card" style="text-align:center;background:linear-gradient(150deg,rgba(127,176,255,.12),var(--glass))">
      <div style="font-size:40px">🔐</div>
...
SCREENS.support = () => `
  <div class="scr">
    ${hdr('Support')}
    <div class="card flex" style="gap:12px">${avx('Care team','', '🤝')}<div class="f1"><div class="row-t">Humans, 24/7, in 6 languages</div><div class="row-d">Median first reply: 38 seconds</div></div><span class="tag grn">Online</span></div>
    <div class="listcard mt12">
...
/* ---------------- agentically better: matched competitor promo ---------------- */
SCREENS['agentic-match'] = () => `
  <div class="scr">
    ${hdr('Agentically better')}
    <div class="card lime">
      <span class="tag solid">Promise kept</span>
      <div class="h1 mt12">Your card just got better. Automatically.</div>
      <div class="sub mt8" style="color:rgba(14,14,16,.66)">FAB refreshed their Cashback Visa campaign yesterday at 14:05. Your Noor Agentic card matched it by 15:02 — applied only to you.</div>
    </div>
    <div class="lbl mt20 mb8">What FAB launched — and what you now have on Noor</div>
    <div class="listcard">
      ${[['5% cashback on groceries & fuel','was 1.5% on Noor','check'],
         ['Double cashback on dining this month','new — matched same-day','check'],
         ['Cinema buy-1-get-1 weekends','matched via Noor Perks','check'],
         ['Annual fee AED 0','always true on Noor','check']]
        .map(([t,d])=>`<div class="row static"><span class="bigico" style="color:var(--grn)">${ic('check',20)}</span><div class="row-main"><div class="row-t" style="white-space:normal">${t}</div><div class="row-d">${d}</div></div></div>`).join('')}
    </div>
    <div class="card soft mt12"><div class="micro">How this works: agents watch every card campaign in the UAE — including banks you already hold. When one of your cards gets better, Noor matches the benefit on your Agentic card within the hour, or simply tells you to pay with theirs. Whichever wins for you. That is the promise: agentically better.</div></div>
    <div class="btnrow mt16">
      <button class="btn lime" onclick="A.toast('Matched benefits active until 31 Jul','check')">Use Noor for these</button>
...
SCREENS.qard = () => {
  ...
  <div class="scr">
    ${hdr('Qard Hasan')}
    <div class="card lime">
      <span class="tag solid">${active?'Active — your salary lands at Noor':'Salary customers'}</span>
      ...
      <div class="sub mt8" style="color:rgba(14,14,16,.66)">${active
        ...
        :'Bring your salary to Noor and hold a Qard Hasan line — the one financing Islam made free by definition. No fee, no profit, no catch. Ever.'}</div>
    </div>
    <div class="lbl mt16 mb8">Your line</div>
    <div class="card">
      <div class="kv"><span class="k">Verified salary${active?' · lands at Noor':''}</span><span class="v tnum">AED ${fm(QARD.salary,0)} / mo</span></div>
    ...
    <div class="card">
      ...
      <div class="kv mt8"><span class="k">Arrives</span><span class="v">in seconds, to your Noor account</span></div>
    ...
    <div class="micro mt8" style="text-align:center">Salary already lands at FAB ··5689 — the letter moves it in one signature. Salary already at Noor? The line is on automatically.</div>`}
  </div>`;
};
window.QardC = {
  draw(){
    ...
    A.sheet(`<div class="h2">Qard Hasan · AED ${fm(amt,0)}</div>
      <div class="listcard mt12">
        <div class="row static"><span class="bigico">${ic('recv',20)}</span><div class="row-main"><div class="row-t">AED ${fm(amt,0)} to Noor current</div><div class="row-d">arrives in seconds</div></div></div>
...
SCREENS['ujrah-card'] = () => {
  ...
  <div class="scr">
    ...
    <div class="card soft mt12"><div class="micro"><b>How Noor earns — said out loud:</b> merchants pay ~${UJRAH.interchange}% interchange on the credit BIN when you spend, plus the flat ujrah. ${UJRAH.note}</div></div>
    <button class="btn lime mt16" onclick="A.toast('Noor Ujrah Card issued — flat AED ${UJRAH.tiers[tier][1]}/mo only when revolving','check')">Issue the card</button>
  </div>`;
};
...
SCREENS['sme-video'] = () => {
  ...
  const stepper = `
    ...
    <div class="mb12">${meter(st/10,'#FF6757')}</div>`;
      ...
      ${SME2.ubos.map((u,k)=>`
        <div class="row" onclick="A.tmp.smeSig=${k};A.refresh()">
          ...
          <div class="row-main"><div class="row-t">${u.n}</div>
            <div class="row-d" style="white-space:normal">${u.role}${k===0?' · already a Noor retail customer — KYC on file':''}</div></div>
    ...
    <div class="card" style="border-color:rgba(31,138,91,.4)">
      <div class="flex" style="gap:12px">
        ...
        <div class="f1"><div class="row-t">Already verified — nothing to upload</div>
          <div class="row-d" style="white-space:normal">Noor retail customer since Jun 2026 · Emirates ID chip-read, ICP match and liveness are on file. Her KYC carries over${sig===0?' — and covers her role as signatory':''}.</div></div>
  ...
  <div class="scr">
    ...
    <div style="border-radius:20px;background:#101014;color:#fff;padding:18px;min-height:250px;display:flex;flex-direction:column;justify-content:space-between;border:1px solid rgba(255,255,255,.12)">
      <div class="flex between">
        <span class="flex" style="gap:7px"><i style="width:10px;height:10px;border-radius:50%;background:${rec?'#ff4d4d':'#666'};display:inline-block"></i><span style="font:600 12px Outfit,Inter,sans-serif">${rec?'REC':'ready'}</span></span>
        <span class="tnum" style="font:600 13px Outfit,Inter,sans-serif" id="smevTimer">${tour?'0:00 / ~1:00':'0:00 / 15:00 max'}</span>
      </div>
      <div style="text-align:center;opacity:.55;font:500 12.5px Outfit,Inter,sans-serif">${tour?'Walk the camera through the real place':'Talk to the camera — like to a partner'}</div>
  ...
  <div class="scr">
    ...
    <div class="card" style="text-align:center;padding:22px 16px">
      <div style="width:64px;height:64px;border-radius:50%;background:#e4f4ec;display:flex;align-items:center;justify-content:center;margin:0 auto;color:#1f8a5b">${ic('check',30)}</div>
      <div class="h1 mt12">We’ve approved your application for AED ${fm(o.limit,0)}</div>
      <div class="sub mt8">Congratulations! Please review the terms of your offer.</div>
    </div>
    <div class="lbl mt16 mb8">Merchant business financing terms</div>
    <div class="card">
      <div class="kv"><span class="k">Financing limit ${tipi('The full Murabaha purchase amount released to you.')}</span><span class="v tnum">AED ${fm(o.limit)}</span></div>
      <div class="kv"><span class="k">Currency</span><span class="v">${o.currency}</span></div>
      <div class="kv"><span class="k">Profit rate ${tipi('Fixed at signing — Murabaha profit is agreed once and can never grow, whatever happens later.')}</span><span class="v tnum">${o.rate}%</span></div>
      <div class="kv"><span class="k">Profit amount ${tipi('The absolute, final dirham cost — AED '+fm(o.profit,0)+', known before you sign.')}</span><span class="v tnum">AED ${fm(o.profit)}</span></div>
      <div class="kv"><span class="k">Minimum instalment ${tipi('Collected weekly by direct debit; pay more anytime.')}</span><span class="v tnum">AED ${fm(o.inst)}/week</span></div>
      <div class="kv"><span class="k">Financing period ${tipi('40 weekly instalments from the start date.')}</span><span class="v">${o.period}</span></div>
      <div class="kv"><span class="k">Start date of financing</span><span class="v">${o.start}</span></div>
      <div class="kv"><span class="k">Final instalment date</span><span class="v">${o.final}</span></div>
      <div class="hr"></div>
      <div class="kv"><span class="k">Transaction type</span><span class="v">${o.type}</span></div>
    ...
    <div class="card">
      ...
      <div class="kv"><span class="k">Unified national number</span><span class="v tnum">${m.unified}</span></div>
      <div class="kv"><span class="k">Disbursement account</span><span class="v tnum" style="font-size:12px">${m.iban}</span></div>
  ...
  <div class="scr">
    ...
    <div class="listcard">
      ${SME2.signdocs.map(([t,ts],k)=>{
        ...
        const done = k < signed;
        return `<div class="row static" style="${isContract?'background:#FFEDEB;border-radius:12px;margin:2px -8px;padding-left:10px;padding-right:10px':''}">
  ...
  <div class="scr">
    ${hdr('Direct debits')}
    ${stepper}
    <div class="sub mb8">Instalments collect themselves — one mandate on the business account, one personal backstop.</div>
    <div class="listcard">
      <div class="row" onclick="A.tmp.smeDD=A.tmp.smeDD||{};A.tmp.smeDD.wio=1;A.refresh();A.toast('Wio mandate signed via UAEDDS','check')">
        ${blg('wio')}
        <div class="row-main"><div class="row-t">Wio Business ··7741 — primary</div>
          <div class="row-d" style="white-space:normal">AED ${fm(SME2.offer.inst,0)}/week · you hold 2 active business accounts — collections stay on this one</div></div>
        ${dd.wio?'<span class="tag grn">signed</span>':'<span class="chev">'+ic('chevR',16)+'</span>'}
      </div>
      <div class="row" onclick="A.tmp.smeDD=A.tmp.smeDD||{};A.tmp.smeDD.enbd=1;A.refresh();A.toast('ENBD personal mandate signed via UAEDDS','check')">
        ${blg('enbd')}
        <div class="row-main"><div class="row-t">ENBD personal ··3021 — backstop</div>
          <div class="row-d" style="white-space:normal">Used only if the business collection fails twice — you approve every retry</div></div>
        ${dd.enbd?'<span class="tag grn">signed</span>':'<span class="chev">'+ic('chevR',16)+'</span>'}
      </div>
    </div>
    <div class="card soft mt12"><div class="micro">Mandates run on UAEDDS — cancellable at your bank anytime. The backstop protects your supplier relationships: a missed week never becomes a missed shipment.</div></div>
    ${both
      ? `<button class="btn lime mt16" onclick="A.toast('AED ${fm(SME2.offer.limit,0)} released to ${SME2.merchant.legal} — first instalment 21 Apr 2025','check')">Release the funds — AED ${fm(SME2.offer.limit,0)}</button>`
      : `<button class="btn ghost mt16" disabled>Sign both mandates to release the funds</button>`}
  </div>`;
  }
  ...
  <div class="scr">
    ...
    <div class="lbl mt16 mb8">What Noor already sees</div>
    <div class="card">
...
window.SMEVC = {
  ...
  sign(){
    ...
    A.tmp.smeSigning=1; A.tmp.smeSigned=A.tmp.smeSigned||0;
    const iv=setInterval(()=>{
      A.tmp.smeSigned++;
      if(A.tmp.smeSigned>=SME2.signdocs.length){ clearInterval(iv); A.tmp.smeSigning=0; }
      A.refresh();
    }, 320);
  }
};
