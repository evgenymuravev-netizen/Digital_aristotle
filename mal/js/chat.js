...
const affordCard = () => `
  <div class="offer-card" style="min-width:0">
    ...
    <div class="mt12">${spark([27,24,31,28,26,33,30,36],300,64,'#1f8a5b')}</div>
...
const SCRIPTS = {
  ...
  ps5:{ user:'I want to buy a PS 5, 2 Tb', async run(c){
    ...
    await c.card(ps5Row());
    await c.card(chips([
      {t:'Split it — Tabby vs Tamara vs Mal', fn:"Pay.buyPS5('Sharaf DG',3150,true)"},
  ...
  ps5Details:{ async run(c){
    await c.ai(`<b>Sharaf DG — AED 3,150</b>: cheapest, delivery in 2–4 h, 2-yr warranty.\n<b>Jumbo — AED 3,249</b>: free HDMI 2.1 cable (worth ~AED 120), tomorrow.\n<b>Amazon.ae — AED 3,329</b>: easiest returns.\n\nFor paying: you’re <b>pre-approved on Tabby (AED 4,500)</b> and <b>Tamara (AED 3,800)</b> — both 0 fees — or <b>Mal Split in 4</b> (Murabaha). Paying in full on FAB earns AED 157 cashback. Tap “Split it” and pick.`);
  }},
  ...
  fx:{ user:'What’s the INR rate today?', async run(c){
    await c.ai(`<b>AED 1 = ₹${FX[0].rate}</b> right now — a 30-day high. Sending ₹2 00,000 today costs <b>AED 8,540</b> (zero Mal fees, mid-market rate). Yesterday it was AED 8,612.`);
    await c.card(chips([
